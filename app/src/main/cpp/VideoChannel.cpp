...
void VideoChannel::video_decode() {
    ...
    while (isPlaying) {
        ...
        ret = avcodec_send_packet(this->avCodecContext, packet); // 第二步：把饭盒给食堂阿姨，食堂阿姨（内部维持一个队列）打菜
        if (ret) {
        ...
        ret = avcodec_receive_frame(this->avCodecContext, frame); //第三步： AVFrame 就有了值了 原始数据==YUV，最终打好的饭菜（原始数据）
...
void VideoChannel::video_player() {
    ...
    SwsContext * sws_ctx = sws_getContext(this->avCodecContext->width, this->avCodecContext->height, this->avCodecContext->pix_fmt, // 原始图形信息相关的 （输入）
                                          this->avCodecContext->width, this->avCodecContext->height, AV_PIX_FMT_RGBA, // 目标 尽量 要和 元素的保持一致（输出）
    ...
    av_image_alloc(dst_data, dst_linesize,
                   this->avCodecContext->width, this->avCodecContext->height, AV_PIX_FMT_RGBA, 1);
    ...
    while(isPlaying) {
        ...
        sws_scale(sws_ctx, frame->data,
                  frame->linesize, 0, this->avCodecContext->height, dst_data, dst_linesize);
        ...
        // 渲染的两种方式：
        // 渲染一帧图像（宽，高，数据）
        this->renderCallback(dst_data[0], this->avCodecContext->width, this->avCodecContext->height , dst_linesize[0]);
